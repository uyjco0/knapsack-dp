...
def solveRecurrence(v, current_pvol, current_pprice, current_pweight, current_pid, table_previous_row):
  """
  ...
  if (current_pvol > v):
     ...
     return table_previous_row[v]
  else:
     if (table_previous_row[v-current_pvol][0] + current_pprice) > table_previous_row[v][0]:
        return (table_previous_row[v-current_pvol][0] + current_pprice, table_previous_row[v-current_pvol][1] + current_pweight, table_previous_row[v-current_pvol][2]+ [current_pid])
     else:
        if (table_previous_row[v-current_pvol][0] + current_pprice) == table_previous_row[v][0]:
...
def fillTable(db_prods):
   ...
   for i in xrange(1, db_prods.getAmountProducts() + 1):
      ...
      # using comprehensions is being a bit slower that using the straight for-loop 
      table_current_row = [solveRecurrence(v, current_pvol, current_pprice, current_pweight, current_pid, table_previous_row) for v in xrange(db_prods.getContainerVolume() + 1)]
      table_previous_row = table_current_row
   return table_current_row[db_prods.getContainerVolume()]
